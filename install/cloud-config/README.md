...
### 配置kubernetes master
1. **基于Discovery server部署etcd2** 使用```etcd2_cc.yaml```配置作为etcd2的集群节点。注意必须配置```name```字段。这里默认是使用discovery方式配置集群(将注释掉的discovery一行加上，并根据说明生成token)
1. **基于Static Mode部署etcd2** 使用```etcd2_cc_static.yaml```配置作为etcd2的集群节点。包含三个etcd2的节点，需要修改```${ipv4_ip}```为当前服务器IP,```${infra0_ip},${infra1_ip},${infra2_ip}```分别为三个节点的IP
